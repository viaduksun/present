...
<nav aria-label="Статьи по Bootstrap 4">
	<form name >
		<ul class="pagination justify-content-center">
			...
			<a onclick="pageGet($page)" type="button" class="page-link" href= ./orders.php?page='. ($page) .'><b>'. ($page) .'</b></a>
			</li>';
			// Проверяем нужны ли 2 стрелки назад
			if ($page > 2) {
				$perv2page = '<li class="page-item">
				<input type="hidden" value="1" id="perv_page">
				<a onclick="pageGet(1)" class="page-link" href= ./orders.php?page=1> << </a></li>';	
			}
			// Проверяем нужна ли стрелка Пред.
			if ($page != 1) {
				$pervpage = '<li class="page-item">
				<input type="hidden" value='. ($page - 1) .' id="pervpage">
				<li class="page-item"><a onclick="pageGet($page - 1)" class="page-link" href= ./orders.php?page='. ($page - 1) .'>Пред.</a></li>';	
			}
    		// Проверяем нужны ли стрелка След.
			if ($page != $total) {
				$nextpage = '<li class="page-item">
				<input type="hidden" value='. ($page + 1) .' id="nextpage">
				<a onclick="pageGet($page + 1)" class="page-link" href= ./orders.php?page='. ($page + 1) .'>След.</a></li>';
			}
    		// Проверяем нужны ли 2 стрелки вперед
			if ($page + 1 < $total) {
				$next2page = '<li class="page-item">
				<input type="hidden" value='. $total .' id="next_page">
				<li class="page-item"><a onclick="pageGet(total)" class="page-link" href= ./orders.php?page=' .$total. '> >> </a></li>';
			}

			// Находим две ближайшие станицы с обоих краев, если они есть
			if($page - 2 > 0) {
				$page2left = '<li class="page-item">
				<a onclick="pageGet($page - 2)" class="page-link" href= ./orders.php?page='. ($page - 2) .'>'. ($page - 2) .'</a>
				</li>';
			}
			if($page - 1 > 0) {
				$page1left = '<li class="page-item">
				<a onclick="pageGet($page - 1)" class="page-link" href= ./orders.php?page='. ($page - 1) .'>'. ($page - 1) .'</a>
				</li>';
			}
			if($page + 2 <= $total) {
