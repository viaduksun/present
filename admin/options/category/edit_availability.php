...
include $_SERVER['DOCUMENT_ROOT'] . "/configs/db.php";
    ...
    $sql = "SELECT * FROM `availability` WHERE `id`=" . $_GET["id"]; 
...
if(isset($_POST["send"])) {
    ...
    if(isset($_POST['newtitle']) && $_POST['newtitle'] != "") { 
       $sql = mysqli_query ($conn, "UPDATE `availability` SET `title`='" . $_POST['newtitle'] . "' WHERE `availability`.id='" . $_GET["id"] . "'" );       
    } 
			...
			<div class="row">
				<div class="col-md-12 text-center">
					<div class="con-text">
							...
							<p><a href="/admin/">Главная</a> | <a href="/admin/pages/category.php">Category</a> | EDIT Availability</p>
					</div>
				</div>
			</div>
	</div>
                                    ...
                                    <div class="easy2">
                                        <table class="table table-hover table-striped">
                                            <div class="container-fluid">
                                                ...
                                                <form action = "/admin/options/category/edit_availability.php?id=<?php echo $_GET["id"];?>" method = "POST">
                                                  
                                                    <label for="formGroupExampleInput">Title availability "<?php echo $row['title']; ?>"</label><br>
                                                    <input type="text" name="newtitle" class="form-control" value="<?php echo $row['title']; ?>" placeholder="<?php echo $row['title']; ?>">
                                                    <br>
                                                    <button class="btn btn-primary ce5 p-2" type="submit" name="send">EDIT Availability</button>
                                                 </form>
                                            </div>
                                        </table>
                                    </div>
